r_v = linspace(14,18,65); %Range to Test
r_w = linspace(-1000,1000,45);    %Range to Test
...
for v_i=1:length(r_v)
    for w_i=1:length(r_w)
        if(r_v(v_i) ~= 0)
            ...
            w = [0 0 r_w(w_i)];   %Initial Angular Velocity
...
ylabel('Initial K Angular Velocity (rad/s)');
xlabel('Inital I Velocity (m/s)');
title('Effect of Sidespin on Successful Serves')